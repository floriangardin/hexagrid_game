...
mountain = pygame.image.load("mountain.jpeg").convert_alpha()


mountain = pygame.Color(151, 124, 83)
desert = pygame.Color(194, 178, 128)
jungle = pygame.Color(41, 171, 135)
ocean = pygame.Color(6, 66, 115)
forest = pygame.Color(34, 139, 34)
tundra = pygame.Color(211, 213, 199)
plain = pygame.Color(86, 125, 70)
hill = pygame.Color(217, 195, 173)
volcano = pygame.Color(78, 39, 40)

COLORS = [mountain, desert, jungle, ocean, forest, tundra, plain, hill, volcano]

probs = [1/len(COLORS) for c in COLORS]



def generate_colors(rows, cols):

    res = [[None] * cols for i in range(rows)]
    rowcols = list(itertools.product(range(rows), range(cols)))
    random.shuffle(rowcols)
    idx = 0
    for row, col in rowcols:
        ...
        if idx > 2:
            break
        res[row][col] = COLORS[choice(range(len(COLORS)), p=probs)]
        idx += 1
...
class Map(object):
    ...
    def __init__( self, rows, cols , *args, **keywords ):
        ...
        self.colors = generate_colors(rows, cols)
